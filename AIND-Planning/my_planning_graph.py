...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for action in self.all_actions:
            ...
            # check if a_node has prenodes that are a subset of the previous S level
            if a_node.prenodes.issubset(prev_state):
                # add a_node to A level
                a_level.add(a_node)
                # connect action node to the S node instances
                for s_node in prev_state:
    ...
    def add_literal_level(self, level):
        ...
        # previous A level
        prev_action = self.a_levels[level - 1]

        for a_node in prev_action:
            # connect S nodes as children of all the A nodes that could produce them, and likewise with A nodes
            for s_node in a_node.effnodes:
                # add S node to the level
